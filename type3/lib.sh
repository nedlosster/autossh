#!/bin/bash
# Хелперы для setup.sh (type3 — multi-host)
...
start_ssh_multiplex() {
    SSH_CONTROL_PATH="/tmp/ssh-tunnel-setup-${HOST_NAME:-$$}"
...
# --- Определение дистрибутива на remote ---
detect_distro_remote() {
    remote_exec cat /etc/os-release | grep '^ID=' | cut -d= -f2 | tr -d '"'
}

# Проверка пакета на remote (с учётом ALT Linux)
_pkg_installed_remote() {
    local pkg="$1"
    local distro
    distro=$(detect_distro_remote)
    if [ "$distro" = "altlinux" ]; then
        remote_exec rpm -q "$pkg" &>/dev/null
    else
        remote_exec dpkg -s "$pkg" &>/dev/null
    fi
}

# Установка пакетов на remote (с учётом ALT Linux)
apt_install_remote() {
    local distro
    distro=$(detect_distro_remote)
    local to_install=()
    for pkg in "$@"; do
        local mapped
        mapped=$(_map_pkg "$pkg" "$distro")
        if ! _pkg_installed_remote "$mapped"; then
            to_install+=("$mapped")
        fi
    done
    if [ ${#to_install[@]} -gt 0 ]; then
        ...
        if [ "$distro" = "altlinux" ]; then
            remote_sudo apt-get update -qq 2>/dev/null || true
            remote_sudo apt-get install -y "${to_install[@]}"
        else
...
# Убедиться, что tunnel-пользователь разрешён в AllowUsers
# Возвращает 0 = изменено, 1 = без изменений
